...
class DataGenerator(object):
    ...
    def gen_batch_in_memory(self, X, X_black, nn_finder, nb_q):
        ...
        while True:
            ...
            X_batch_ab = X_batch_color[:, 1:, :, :]
            X_batch_l = X_batch_color[:, :1, :, :]

            npts, c, h, w = X_batch_ab.shape
            X_a = np.ravel(X_batch_ab[:, 0, :, :])
            X_b = np.ravel(X_batch_ab[:, 1, :, :])
            X_batch_ab_variant = np.vstack((X_a, X_b)).T

            Y_batch = self.get_soft_encoding(X_batch_ab_variant, nn_finder, nb_q)

            Y_batch = Y_batch.reshape((npts, h, w, nb_q))

            yield X_batch_black, X_batch_color, Y_batch, X_batch_l, X_batch_ab
